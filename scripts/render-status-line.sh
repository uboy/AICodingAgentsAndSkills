...
if root:
    # Token usage from session tracker
    usage_file = Path(root) / "coordination" / "state" / "session-usage.json"
    if usage_file.is_file():
        try:
            usage = json.loads(usage_file.read_text(encoding="utf-8"))
            est = usage.get("estimated_tokens_used", 0) or 0
            if est > 0:
                if est >= 1_000_000:
                    tokens = f"{est/1_000_000:.1f}M"
                elif est >= 1_000:
                    tokens = f"{est/1_000:.1f}K"
                else:
                    tokens = str(est)
            pct = usage.get("estimated_usage_percent", 0) or 0
            if pct > 0:
                limit = f"{pct:.0f}%"
        except Exception:
            pass

    # Retry count from agent state
    state_file = Path(root) / "coordination" / "state" / f"{system}.md"
    retry_val = read_state_value(state_file, "retry_count")
    if retry_val and retry_val != "0":
        retry = f"R{retry_val}"
    status_val = read_state_value(state_file, "status")
    if status_val == "rate_limited":
        limit = "LIMIT"

line = (
    f"{shorten(model, 20)} | ag:{shorten(agent, 12)} | sk:{shorten(skill, 14)} | "
    f"step:{shorten(step, 28)} | chk:{done}/{total} | task:{shorten(task_id, 18)} | "
    f"ctx:{ctx} | tok:{tokens} | lim:{limit} | {retry}"
)
print(line)
PY
