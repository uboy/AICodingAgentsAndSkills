...
for rel in "${adapter_files[@]}"; do
  ...
  if grep -qF "{{SYSTEM_LABEL}}" "$path"; then
    add_result "FAIL" "adapter-template" "UNRESOLVED TEMPLATE: $rel contains {{SYSTEM_LABEL}}"
  fi
  if grep -qF "{{EXTRA_FOOTER}}" "$path"; then
    add_result "FAIL" "adapter-template" "UNRESOLVED TEMPLATE: $rel contains {{EXTRA_FOOTER}}"
  fi
done
