...
function Add-AllowedTarget([hashtable]$AllowedTargets, [string]$Path) {
    $AllowedTargets[(Get-Abs $Path)] = $true
}

function Add-AllowedCoverage([hashtable]$AllowedTargets, [string]$SourcePath, [string]$TargetPath) {
    if (-not (Test-Path -LiteralPath $SourcePath)) {
        return
    }

    if (Test-Path -LiteralPath $SourcePath -PathType Leaf) {
        Add-AllowedTarget -AllowedTargets $AllowedTargets -Path $TargetPath
        return
    }
    ...
    if (Test-Path -LiteralPath $SourcePath -PathType Container) {
        Get-ChildItem -LiteralPath $SourcePath -Recurse -File | ForEach-Object {
            ...
            Add-AllowedTarget -AllowedTargets $AllowedTargets -Path (Join-Path $TargetPath $rel)
        }
    }
}
...
$allowedTargets = @{}
$reportedExtraTargets = @{}

foreach ($entry in $entries) {
    Add-AllowedCoverage -AllowedTargets $allowedTargets -SourcePath (Join-Path $RepoRoot $entry.Source) -TargetPath (Join-Path $HomeDir $entry.Target)
}
    ...
    if (Test-Path -LiteralPath $sourcePath -PathType Container) {
        ...
        if (Test-Path -LiteralPath $targetPath -PathType Container) {
            Get-ChildItem -LiteralPath $targetPath -Recurse -File | ForEach-Object {
                ...
                $normalizedTarget = Get-Abs $_.FullName
                if ((-not $relativeSourceFiles.ContainsKey($rel)) -and (-not $allowedTargets.ContainsKey($normalizedTarget)) -and (-not $reportedExtraTargets.ContainsKey($normalizedTarget))) {
                    $reportedExtraTargets[$normalizedTarget] = $true
