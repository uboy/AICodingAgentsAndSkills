...
$useScopeFile = $changed -contains $scopeRel
if (-not (Test-Path -LiteralPath $scopePath -PathType Leaf)) {
    Add-Issue -Severity "PASS" -Check "scope-file" -Detail ("No local scope file detected at {0}; scope-drift check skipped." -f $scopeRel)
} elseif (-not $useScopeFile) {
    Add-Issue -Severity "PASS" -Check "scope-file" -Detail ("Local scope file exists at {0} but is not part of the current change set; scope-drift check skipped." -f $scopeRel)
} else {
...
if ($useScopeFile -and $scopePatterns.Count -gt 0 -and $changed.Count -gt 0) {
...
} elseif ($useScopeFile -and $changed.Count -gt 0 -and $scopePatterns.Count -gt 0) {
...
if ($functionalChanged.Count -gt 0 -and (-not $isTrivialConfigOnly)) {
    ...
    if ($hasTaskUpdate) {
        Add-Issue -Severity "PASS" -Check "tasks-checklist" -Detail "Local coordination/tasks.jsonl updated."
    } else {
        Add-Issue -Severity "PASS" -Check "tasks-checklist" -Detail "Tracked task ledger not required; no local tasks.jsonl update detected."
    }
    if ($hasHandoffInChanges) {
        Add-Issue -Severity "PASS" -Check "handoff-evidence" -Detail "Local handoff evidence detected."
    } else {
        Add-Issue -Severity "PASS" -Check "handoff-evidence" -Detail "Tracked handoff artifacts are optional; no local handoff update detected."
    }
    if ($reviewFiles.Count -gt 0) {
        ...
        Add-Issue -Severity "PASS" -Check "review-pipeline" -Detail "Tracked review reports are optional; no local review report update detected."
    }
