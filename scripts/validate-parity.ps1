...
# Build artifacts live in out/ (generated, not tracked in VCS)
$outDir = Join-Path $RepoRoot "out"

# All adapter files are now generated into out/ by sync-adapters
$adapterFiles = @(
    @{ Rel = "out/CLAUDE.md"; Source = "out" }
    @{ Rel = "out/CURSOR.md"; Source = "out" }
    @{ Rel = "out/GEMINI.md"; Source = "out" }
    @{ Rel = "out/OPENCODE.md"; Source = "out" }
...
$adapterLineLimitOverrides = @{
    "out/.codex/AGENTS.md" = 1000
}

$rawTemplateMarkers = @("{{SYSTEM_LABEL}}", "{{EXTRA_FOOTER}}")
...
foreach ($entry in $adapterFiles) {
    ...
    foreach ($marker in $rawTemplateMarkers) {
        if (($lines -join "`n").Contains($marker)) {
            Add-Result -Severity "FAIL" -Check "adapter-template" -Detail ("{0} contains unresolved template marker: {1}" -f $rel, $marker)
        }
    }
}
