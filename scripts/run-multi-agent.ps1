...
$coordDirs = @(
    ...
    (Join-Path $RepoRoot "coordination\code_map"),
    (Join-Path $RepoRoot "coordination\handoffs"),
    (Join-Path $RepoRoot "coordination\reviews"),
...
$sessionUsageFile = Join-Path $RepoRoot "coordination\state\session-usage.json"
$tasksFile = Join-Path $RepoRoot "coordination\tasks.jsonl"
if (-not (Test-Path -LiteralPath $tasksFile)) {
    Invoke-Exec { New-Item -ItemType File -Path $tasksFile -Force | Out-Null } "Create local task tracker: $tasksFile"
}
if (-not (Test-Path -LiteralPath $sessionUsageFile)) {
    $seedAgent = if ($agentList.Count -gt 0) { $agentList[0] } else { "codex" }
    $sessionUsageContent = New-SessionUsageContent -AgentName $seedAgent
    Invoke-Exec { Set-Content -LiteralPath $sessionUsageFile -Value $sessionUsageContent } "Create session usage file: $sessionUsageFile"
}
...
Write-Step "Local task tracker: $tasksFile"
Write-Step "Session usage file: $sessionUsageFile"
