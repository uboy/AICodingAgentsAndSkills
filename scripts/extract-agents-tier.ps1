...
# Usage: .\scripts\extract-agents-tier.ps1 [-OutDir <path>] [-DryRun] [-Check]
#
...
# Resolve output directory: default to out/, or use explicit path
if ([string]::IsNullOrWhiteSpace($OutDir)) {
    $outDirResolved = Join-Path $repoRoot 'out'
} else {
    $outDirResolved = $OutDir
}

$outHot     = Join-Path $outDirResolved 'AGENTS-hot.md'
$outWarm    = Join-Path $outDirResolved 'AGENTS-warm.md'
$outCold    = Join-Path $outDirResolved 'AGENTS-cold.md'
$outHotWarm = Join-Path $outDirResolved 'AGENTS-hot-warm.md'
...
# ---------------------------------------------------------------------------
# Helper: write or check one tier file
# ---------------------------------------------------------------------------
...
if (-not $DryRun -and -not $Check) {
    ...
    Write-Host "  Output directory:  $outDirResolved"

    # Auto-sync adapter files — generates ALL system files from adapters/
    $SyncScript = Join-Path $repoRoot "scripts/sync-adapters.ps1"
    if (Test-Path $SyncScript) {
        Write-Host ""
        Write-Host "Running sync-adapters.ps1 to generate all system files..."
        & pwsh -File $SyncScript -OutDir $outDirResolved
    }
}
