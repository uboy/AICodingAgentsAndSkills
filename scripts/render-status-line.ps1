...
function Resolve-RepoRoot([string]$Hint) {
    if (-not [string]::IsNullOrWhiteSpace($Hint)) {
        if ((Test-Path -LiteralPath (Join-Path $Hint "AGENTS.md") -PathType Leaf) -or (Test-Path -LiteralPath (Join-Path $Hint ".git"))) {
            return (Resolve-Path -LiteralPath $Hint).Path
        }
    }

    if (-not [string]::IsNullOrWhiteSpace($env:AI_AGENT_REPO_ROOT)) {
        if ((Test-Path -LiteralPath (Join-Path $env:AI_AGENT_REPO_ROOT "AGENTS.md") -PathType Leaf) -or (Test-Path -LiteralPath (Join-Path $env:AI_AGENT_REPO_ROOT ".git"))) {
    ...
    while ($true) {
        if ((Test-Path -LiteralPath (Join-Path $dir "AGENTS.md") -PathType Leaf) -or (Test-Path -LiteralPath (Join-Path $dir ".git"))) {
            return $dir
        }
...
# Read token usage from session tracker
if (-not [string]::IsNullOrWhiteSpace($root)) {
    $usagePath = Join-Path $root "coordination\state\session-usage.json"
    if (Test-Path $usagePath) {
        try {
            $usage = Get-Content $usagePath -Raw | ConvertFrom-Json
            $estTokens = $usage.estimated_tokens_used
            if ($estTokens -ne $null -and $estTokens -gt 0) {
                if ($estTokens -ge 1000000) { $tokens = ("{0:0.0}M" -f ($estTokens / 1000000)) }
                elseif ($estTokens -ge 1000) { $tokens = ("{0:0.0}K" -f ($estTokens / 1000)) }
                else { $tokens = [string]$estTokens }
            }
            $usagePct = $usage.estimated_usage_percent
            if ($usagePct -ne $null -and $usagePct -gt 0) {
                $limit = ("{0:0}%" -f [double]$usagePct)
            }
        } catch {}
    }

    # Read retry count from agent state
    $statePath = Join-Path $root ("coordination\state\{0}.md" -f $System)
    $retryVal = Parse-StateValue -Path $statePath -Key "retry_count"
    if (-not [string]::IsNullOrWhiteSpace($retryVal) -and $retryVal -ne "0") {
        $retry = ("R{0}" -f $retryVal)
    }
    $statusVal = Parse-StateValue -Path $statePath -Key "status"
    if ($statusVal -eq "rate_limited") {
        $limit = "LIMIT"
    }
}

$line = "{0} | ag:{1} | sk:{2} | step:{3} | chk:{4}/{5} | task:{6} | ctx:{7} | tok:{8} | lim:{9} | {10}" -f `
    (Shorten $model 20), `
    (Shorten $agent 12), `
    (Shorten $skill 14), `
    (Shorten $step 28), `
