...
declare -A REPORTED_EXTRA_TARGETS=()
...
add_allowed_coverage() {
  ...
  if [[ -f "$source_path" ]]; then
    add_allowed_target "$target_path"
    return
  fi
  ...
  if [[ -d "$source_path" ]]; then
    while IFS= read -r src_file; do
      ...
      add_allowed_target "$target_path/$rel"
...
while IFS='|' read -r source_rel target_rel; do
  ...
  add_allowed_coverage "$REPO_ROOT/$source_rel" "$HOME_DIR/$target_rel"
done < "$MANIFEST_PATH"
...
if [[ -f "$SKILL_MANIFEST_PATH" ]]; then
  while IFS=$'\t' read -r action kind system tier source_rel target_rel _rest; do
    ...
    source_rel="$(echo "${source_rel:-}" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')"
    target_rel="$(echo "${target_rel:-}" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')"
    if [[ "$action" != "deploy" || -z "$source_rel" || -z "$target_rel" ]]; then
      continue
    fi
    add_allowed_coverage "$REPO_ROOT/$source_rel" "$HOME_DIR/$target_rel"
  ...
  if [[ -d "$source_path" ]]; then
    ...
    if [[ -d "$target_path" ]]; then
      while IFS= read -r tgt_file; do
        ...
        if [[ -z "${SRC_REL["$rel"]+x}" && -z "${ALLOWED_TARGETS["$tgt_file"]+x}" && -z "${REPORTED_EXTRA_TARGETS["$tgt_file"]+x}" ]]; then
          REPORTED_EXTRA_TARGETS["$tgt_file"]=1
