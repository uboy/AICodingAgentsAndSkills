...
for rel in "${changed[@]}"; do
  ...
  if [[ "$rel" == *.sh ]]; then
    if ! "$SHELL_BIN" -n "$path" >/dev/null 2>&1; then
...
if [[ ${#coordination_changed[@]} -gt 0 ]]; then
  if [[ -f "$REPO_ROOT/scripts/validate-coordination.sh" ]]; then
    coord_args=(--repo-root "$REPO_ROOT")
    for rel in "${coordination_changed[@]}"; do
      coord_args+=(--files-to-validate "$rel")
    done
    if ! "$SHELL_BIN" "$REPO_ROOT/scripts/validate-coordination.sh" "${coord_args[@]}" >/dev/null 2>&1; then
...
cycle_artifacts_changed=($(printf "%s\n" "${changed[@]}" | grep -E '^coordination/(cycle-contract\.json|reviews/|handoffs/)' || true))

# Skills validation
if [[ $skills_changed -eq 1 ]]; then
  if [[ -f "$REPO_ROOT/scripts/validate-skills.sh" ]]; then
    if ! "$SHELL_BIN" "$REPO_ROOT/scripts/validate-skills.sh" >/dev/null; then
...
if [[ -f "$REPO_ROOT/scripts/run-integrity-fast.sh" ]]; then
  integrity_output="$("$SHELL_BIN" "$REPO_ROOT/scripts/run-integrity-fast.sh" 2>&1)" || {
    echo "$integrity_output" >&2
    add_issue "FAIL" "integrity-fast" "run-integrity-fast.sh failed (see output above)."
    integrity_output=""
  }
  if [[ -n "$integrity_output" ]]; then
    add_issue "PASS" "integrity-fast" "run-integrity-fast.sh passed."
  fi
else
...
if [[ -f "$REPO_ROOT/scripts/change-control-gate.sh" ]]; then
  change_control_output="$("$SHELL_BIN" "$REPO_ROOT/scripts/change-control-gate.sh" 2>&1)" || {
...
if [[ ${#cycle_artifacts_changed[@]} -gt 0 && -f "$REPO_ROOT/scripts/validate-cycle-proof.sh" ]]; then
  cycle_output="$("$SHELL_BIN" "$REPO_ROOT/scripts/validate-cycle-proof.sh" 2>&1)" || {
    echo "$cycle_output" >&2
...
elif [[ ${#cycle_artifacts_changed[@]} -gt 0 ]]; then
  add_issue "FAIL" "cycle-proof" "scripts/validate-cycle-proof.sh not found."
else
  add_issue "PASS" "cycle-proof" "No local cycle-proof artifacts changed; skipped."
fi
