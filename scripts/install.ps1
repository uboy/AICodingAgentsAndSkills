...
# ---------------------------------------------------------------------------
# Build step: generate ALL output files (tiers + adapters + agents) into out/.
# This ensures all manifest source paths resolve correctly.
# ---------------------------------------------------------------------------
Write-Step "Building all output files into out/"
$SyncScript = Join-Path $PSScriptRoot "sync-adapters.ps1"
if (Test-Path $SyncScript) {
    & pwsh -NoProfile -File $SyncScript -OutDir (Join-Path $RepoRoot 'out') | Out-Host
} else {
    Write-Warn "sync-adapters.ps1 not found — skipping build step"
}
