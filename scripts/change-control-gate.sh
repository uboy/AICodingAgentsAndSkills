...
if [[ ! -f "$scope_path" ]]; then
  add_issue "PASS" "scope-file" "No local scope file detected at $scope_rel; scope-drift check skipped."
elif [[ $use_scope_file -eq 0 ]]; then
  add_issue "PASS" "scope-file" "Local scope file exists at $scope_rel but is not part of the current change set; scope-drift check skipped."
else
  while IFS= read -r line; do
...
if [[ $use_scope_file -eq 1 && ${#scope_patterns[@]} -gt 0 && ${#changed[@]} -gt 0 ]]; then
  for rel in "${changed[@]}"; do
...
elif [[ $use_scope_file -eq 1 && ${#changed[@]} -gt 0 && ${#scope_patterns[@]} -gt 0 ]]; then
...
if [[ $functional_changed -eq 1 && $is_trivial_config_only -eq 0 ]]; then
  ...
  if [[ $has_tasks -eq 1 ]]; then
    add_issue "PASS" "tasks-checklist" "Local coordination/tasks.jsonl updated."
  else
    add_issue "PASS" "tasks-checklist" "Tracked task ledger not required; no local tasks.jsonl update detected."
  fi

  if [[ $has_handoff_in_changes -eq 1 ]]; then
    add_issue "PASS" "handoff-evidence" "Local handoff evidence detected."
  else
    add_issue "PASS" "handoff-evidence" "Tracked handoff artifacts are optional; no local handoff update detected."
  fi

  if [[ $has_review -eq 1 ]]; then
    if [[ -f "$REPO_ROOT/scripts/validate-review-report.sh" ]]; then
      if "$SHELL_BIN" "$REPO_ROOT/scripts/validate-review-report.sh" >/dev/null 2>&1; then
    ...
    add_issue "PASS" "review-pipeline" "Tracked review reports are optional; no local review report update detected."
  fi
