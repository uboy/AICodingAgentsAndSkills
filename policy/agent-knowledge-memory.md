...
This policy defines how to retain and revalidate reusable local operational knowledge
...
`.agent-memory/` is local-only runtime support for the current machine/user and is not
repository content.

If a lesson should become shared repository guidance, write it into tracked docs,
policy files, or design artifacts instead of `.agent-memory/`.
