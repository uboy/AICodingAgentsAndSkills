...
Common model mistakes observed during code review:

- **Nitpicking over substance**: The model focuses on formatting/style issues instead of real bugs. Prioritize correctness, security, and logic errors first.
- **Hallucinated edge cases**: The model invents unrealistic scenarios ("what if this runs during a leap year on a Tuesday?"). Only flag edge cases that are plausible in production.
- **Ignoring the diff context**: The model reviews unchanged code around the diff instead of the actual changes. Focus ONLY on what was added/modified/removed.
- **Generic fix suggestions**: The model says "add error handling" without showing HOW. Every finding must include a concrete fix with code or pattern.
- **Missing security blind spots**: The model catches logic bugs but misses injection, auth bypass, data exposure. Always scan input validation, auth checks, and error message content.
- **Severity inflation**: The model marks style issues as "Critical". Reserve Critical for bugs that will definitely cause production failure or security breach.
