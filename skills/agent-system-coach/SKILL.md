...
3. If a proposed update changes architecture or existing tests, require explicit user approval and record it in local `coordination/approval-overrides.json`.
...
5. Teach scope discipline:
   - If using the local coordination workflow, update `coordination/change-scope.txt` before implementation.
   - Keep edits inside declared scope when that local scope file is in use.
