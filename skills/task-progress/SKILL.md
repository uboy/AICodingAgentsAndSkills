...
If input is incomplete, ask focused clarifying questions in the user's language.
...
3. For each materially missing area, ask **one focused question in the user's language**.
...
### Question bank — localize to the user's language when the area is missing or too vague

**Done:**
- "What exactly was implemented or fixed since the last update?"
- "How did you verify that the change works correctly? (unit test, device run, CI)"

**Next:**
- "What is the next concrete step, and is there an ETA?"
- "Is there a dependency on another task or a pending review?"

**Risks — ArkUI/OpenHarmony-specific:**
- "Did the public API of the component or module interface change?" → backward-compat
- "Did this touch ArkUI rendering or the component tree? Could it affect FPS?" → rendering-perf
- "Are large lists involved (`ForEach` > 50 items)?" → list-perf (`LazyForEach`)
- "Do `@State` / `@Observed` updates go deeper than one nesting level?" → state-observation
- "Does this affect Ability lifecycle (`onBackground`, `onDestroy`)?" → lifecycle / resource leak
- "Are third-party native libraries involved (`.so`, NDK, FFI)?" → native-dep
- "Which devices / OpenHarmony versions were used for testing?" → device-compat

**Artifacts:**
- "Is there a commit hash, PR link, or document to attach?"
